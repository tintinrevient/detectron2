...
def _draw_one_rotated_and_scaled_segm(segm_id, segm_xy, midpoint, scaler):
    ...
    # debug
    # img_test = np.empty((max_y, max_x, 4), np.uint8)
    # img_test.fill(255)
    # # segment
    # for x, y in segm_xy:
    #     cv2.circle(img_test, (int(x), int(y)), 1, COARSE_TO_COLOR[segm_id], -1)
    # # midpoint
    # cv2.circle(img_test, (int(midpoint[0]), int(midpoint[1])), 5, (255, 0, 255), -1)
    # # bbox
    # rect_xy = _get_min_bounding_rect(segm_xy)
    # cv2.line(img_test, (int(rect_xy[0][0]), int(rect_xy[0][1])), (int(rect_xy[1][0]), int(rect_xy[1][1])), (0, 255, 0), 5)
    # cv2.line(img_test, (int(rect_xy[1][0]), int(rect_xy[1][1])), (int(rect_xy[2][0]), int(rect_xy[2][1])), (0, 255, 0), 5)
    # cv2.line(img_test, (int(rect_xy[2][0]), int(rect_xy[2][1])), (int(rect_xy[3][0]), int(rect_xy[3][1])), (0, 255, 0), 5)
    # cv2.line(img_test, (int(rect_xy[3][0]), int(rect_xy[3][1])), (int(rect_xy[0][0]), int(rect_xy[0][1])), (0, 255, 0), 5)
    # # show
    # cv2.imshow(segm_id, img_test)
    # cv2.waitKey()
    # cv2.destroyAllWindows()
...
def visualize(infile, score_cutoff, gender):
    ...
    for person_densepose, person_box_xywh, person_keypoints in zip(people_densepose,people_box_xywh, people_keypoints):
        ...
        if _is_valid(person_keypoints):
            ...
            outfile = _generate_norm_segm_outfile(infile, person_index)
...
def _generate_norm_segm_outfile(infile, person_index):
...
if __name__ == '__main__':
    ...
    # python visualize_rect_segm.py --input datasets/modern/Paul\ Gauguin/30963.jpg --gender woman

    # classical
    # python visualize_rect_segm.py --input datasets/classical/Michelangelo/12758.jpg --gender man
    # python visualize_rect_segm.py --input datasets/classical/Artemisia\ Gentileschi/45093.jpg --gender man
    ...
    visualize(infile=args.input, score_cutoff=0.95, gender=args.gender)
